...
対話セッション用のリソースを`salloc`コマンドで確保します。次の例では、2GPUを10分間要求します。出力メッセージの`2081`はジョブIDです。
...
投入済みまたは実行中のジョブを`scancel`コマンドでキャンセルします。その際、`squeue`やジョブ投入時に表示されるジョブIDを指定します。
